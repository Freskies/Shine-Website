				...
				<div className="bg-zinc-100 rounded-[2em] p-[2em] border border-zinc-200 mb-[2em] overflow-hidden">
					<div className="mb-[1.5em]">
						<div className="relative bg-zinc-200 p-1 rounded-full w-fit mx-auto flex">
							...
							>
								...
								<span className={`relative z-10 ${bookingType === 'manual' ? 'text-white' : 'text-zinc-600'}`}>{t.bookLesson.manual}</span>
							</button>
							<button
							...
							>
								...
								<span className={`relative z-10 ${bookingType === 'online' ? 'text-white' : 'text-zinc-600'}`}>{t.bookLesson.online}</span>
							</button>
						</div>
					</div>
					...
					<AnimatePresence mode="wait">
						{bookingType === 'manual' ? (
							...
							>
								{steps.map((step) => (
									<div key={step.id} className="flex gap-[1.5em] items-center p-[2em] bg-white rounded-[2em] shadow-sm border border-zinc-100">
										<div className="bg-accent text-white w-[2.5em] h-[2.5em] rounded-full flex items-center justify-center font-bold flex-shrink-0 text-[1.2em]">
											{step.id}
										</div>
										<div>
							...
							{selectedCourse && (
								...
								>
									<div className="flex flex-col md:flex-row gap-[1.5em] items-center text-center md:text-left">
										<div className="w-[5em] h-[5em] bg-zinc-100 rounded-full overflow-hidden relative flex-shrink-0 border-2 border-white shadow-md">
											<Image src="/logo_shine_circle.avif" alt={selectedCourse.instructor} fill className="object-cover" />
										</div>
										<div>
